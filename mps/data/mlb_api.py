...
No API key is required.  Raw JSON responses are cached on disk so repeated
ingestion runs do not hammer the API.  All parsing functions are pure so they
can be unit-tested against fixture payloads without network access.
"""
...
# JSON keys returned by the API -> our column names.
_BATTING_MAP = {
...
class MLBStatsClient:
    ...
    def get(self, path: str, params: dict[str, Any] | None = None, cache_key: str | None = None) -> dict:
        params = params or {}
    ...
    # ------------------------------------------------------------- endpoints
    def schedule(self, start_date: str, end_date: str, game_types: Iterable[str] = ("R",)) -> list[dict]:
        """Return flattened schedule rows between two ISO dates (inclusive)."""
        payload = self.get(
            "schedule",
            {"sportId": 1, "startDate": start_date, "endDate": end_date,
             "gameType": ",".join(game_types), "hydrate": "probablePitcher"},
            cache_key=f"schedule_{start_date}_{end_date}_{'_'.join(game_types)}",
        )
        return parse_schedule(payload)

    def boxscore(self, game_pk: int) -> dict:
        return self.get(f"game/{game_pk}/boxscore", cache_key=f"boxscore_{game_pk}")
    ...
    def players(self, season: int) -> list[dict]:
        """All players on MLB rosters for a season (used for name -> id lookups)."""
        payload = self.get(f"sports/1/players", {"season": season}, cache_key=f"players_{season}")
        return [
            {
                "player_id": p["id"],
        ...
        for entry in (team.get("players") or {}).values():
            ...
            if bat and (bat.get("plateAppearances", 0) or bat.get("atBats", 0)):
                ...
                row = {
                    ...
                    "batting_order": int(str(order)[0]) if order else 0,
