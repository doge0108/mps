...
def test_fetch_in_progress_season_stops_at_today(small_dataset, tmp_path):
    ds = small_dataset
    ...
    assert got.games["temp_f"].notna().all()
...
def test_update_adds_new_finals_and_upcoming_games_with_lineups(small_dataset, tmp_path):
    ds = small_dataset
